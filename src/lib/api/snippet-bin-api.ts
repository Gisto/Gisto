...
export const SnippetBinApi: SnippetProvider<SnippetBinGist, SnippetBinGist> = {
  ...
  get baseUrl() {
    const url = globalState.getState().settings.snippetBinBaseUrl;
  ...
  async fetchSnippets(): Promise<{
    ...
    return {
      nodes: allGists.map((gist) => ({
        ...this.mapToSnippetType(gist),
  ...
  mapToSnippetType(data: SnippetBinGist): SnippetType {
    ...
    Object.values(data.files).forEach((file) => {
      const extension = file.filename.split('.').reverse()[0]?.toLowerCase() || '';
      files[file.filename] = {
        filename: file.filename,
        type: file.type || this.guessMimeType(extension),
        language: file.language,
        raw_url: file.raw_url
          ? `${this.baseUrl.replace(/\/api$/, '')}${file.raw_url}`
          : `${this.baseUrl.replace(/\/api$/, '')}/api/gists/${data.id}/files/${encodeURIComponent(file.filename)}`,
        size: file.size ?? 0,
        truncated: false,
