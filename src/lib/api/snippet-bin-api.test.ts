...
describe('SnippetBinApi', () => {
  ...
  describe('mapToSnippetType', () => {
    it('should handle minimal file data from list endpoint (missing raw_url, type, size)', () => {
      ...
      expect(mapped.id).toBe('gist456');
      expect(mapped.files['script.py'].raw_url).toBe(
        'http://localhost:3001/api/gists/gist456/files/script.py'
      );
      expect(mapped.files['script.py'].type).toBe('text/plain');
      expect(mapped.files['script.py'].size).toBe(0);
      expect(mapped.files['data.json'].raw_url).toBe(
        'http://localhost:3001/api/gists/gist456/files/data.json'
      );
      expect(mapped.files['data.json'].type).toBe('application/json');
      expect(mapped.files['data.json'].size).toBe(0);
    });
