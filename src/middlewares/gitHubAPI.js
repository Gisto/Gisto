...
import { defaultAPIEndpointURL } from 'constants/config';
...
const gitHubAPIMiddleware = ({ dispatch }) => {
  return (next) => (action) => {
    if (action.type === AT.GET_USER) {
      ...
      API.get(`${defaultAPIEndpointURL}/user`)
        .set(_headers())
        .end((error, result) => {
          if (error) {
            dispatch({
    ...
    if (action.type === AT.GET_STARRED_SNIPPETS) {
      ...
      return API.get(`${defaultAPIEndpointURL}/gists/starred`)
        .set(_headers())
        .end((error, result) =>
    ...
    if (action.type === AT.GET_SNIPPETS) {
      ...
      const getGists = (page) => API.get(`${defaultAPIEndpointURL}/gists?page=${page}&per_page=100`)
        .set(_headers())
        .end((error, result) => {
          if (error) {
            dispatch({
    ...
    if (action.type === AT.GET_SNIPPET) {
      ...
      return API.get(`${defaultAPIEndpointURL}/gists/${action.payload.id}`)
        .set(_headers())
        .end((error, result) => {
          if (error) {
            dispatch({
