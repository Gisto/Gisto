...
data.lastUpdated=new Date();appSettings.setOne('avatar',data[item].user.gravatar_id);gistData.list.push.apply(gistData.list,data);var header=headers();if(header.link){var links=header.link.split(',');for(var link in links){link=links[link];if(link.indexOf('rel="next')>-1){var nextPage=link.match(/[0-9]+/)[0];if(!pageNumber||nextPage>pageNumber){api.gists(null,nextPage);return;}}}}
api.starred(function(response){for(var s in response.data){var gist=gistData.getGistById(response.data[s].id);if(gist){gist.has_star=true;}}});}).error(function(data,status,headers,config){console.log({data:data,status:status,headers:headers(),config:config});});},gist:function(id){var gist=gistData.getGistById(id);requestHandler({method:'GET',url:api_url+'/'+id,headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){api.is_starred(data.id,function(response){if(response.status===204){data.starred=true;}else{data.starred=false;}
console.log('Is it starred: '+data.starred);});data.lastUpdated=new Date();console.log(data.lastUpdated);gist.single=data;}).error(function(data,status,headers,config){console.log({data:data,status:status,headers:headers(),config:config});});},create:function(data,callback){requestHandler({method:'POST',url:api_url,data:data,headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},edit:function(id,data,callback){requestHandler({method:'PATCH',url:api_url+'/'+id,data:data,headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},delete:function(id,callback){requestHandler({method:'DELETE',url:api_url+'/'+id,headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},comments:function(id,callback){requestHandler({method:'GET',url:api_url+'/'+id+'/comments',headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},starred:function(callback,pageNumber){var url=pageNumber?api_url+'/starred'+'?page='+pageNumber:api_url+'/starred';requestHandler({method:'GET',url:url,headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){callback({data:data,status:status,headers:headers(),config:config});var header=headers();if(header.link){var links=header.link.split(',');for(var link in links){link=links[link];if(link.indexOf('rel="next')>-1){var nextPage=link.match(/[0-9]+/)[0];if(!pageNumber||nextPage>pageNumber){api.starred(callback,nextPage);return;}}}}}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},star:function(id,callback){requestHandler({method:'PUT',url:api_url+'/'+id+'/star',headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},unstar:function(id,callback){requestHandler({method:'DELETE',url:api_url+'/'+id+'/star',headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},is_starred:function(id,callback){requestHandler({method:'get',url:api_url+'/'+id+'/star',headers:{Authorization:'token '+token}}).success(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});}).error(function(data,status,headers,config){return callback({data:data,status:status,headers:headers(),config:config});});},fork:function(){}};return api;});});angular.module('gistData',[],function($provide){$provide.factory('gistData',function(){var dataService={list:[],getGistById:function(id){for(var gist in dataService.list){gist=dataService.list[gist];if(gist.id===id){return gist;}}}};return dataService;});});angular.module('appSettings',[],function($provide){$provide.factory('appSettings',function(){var settings={theme_list:['default','gisto','nite','dark','dark-blue'],editor_theme_list:['ambiance','chaos','chrome','clouds','clouds_midnight','cobalt','crimson_editor','dawn','dreamweaver','eclipse','github','idle_fingers','kr','merbivore','merbivore_soft','mono_industrial','monokai','pastel_on_dark','solarized_dark','solarized_light','textmate','tomorrow','tomorrow_night_blue','tomorrow_night_bright','tomorrow_night_eighties','tomorrow_night','twilight','vibrant_ink','xcode'],isLoggedIn:function(callback){if(localStorage.settings&&JSON.parse(localStorage.settings).token!==undefined){return true;}else{document.location.href='#/login';}},logOut:function(){delete localStorage.settings;document.location.href='#/login';},getAll:function(){var all_settings=JSON.parse(localStorage.settings);return all_settings;},get:function(name){if(settings.isLoggedIn()){var storage=JSON.parse(localStorage.settings);return storage[name];}},set:function(data,callback){var settings=JSON.parse(localStorage.settings)||{};for(var key in data){settings[key]=data[key];}
settings.last_modified=new Date().toUTCString();localStorage.settings=JSON.stringify(settings);if(callback){return callback({status:'ok'});}},setOne:function(key,new_data,callback){var old_data=settings.getAll();old_data[key]=new_data;settings.set(old_data);}};return settings;});});'use strict';function settingsCtrl($scope,appSettings){$scope.themes=appSettings.theme_list;$scope.editor_themes=appSettings.editor_theme_list;$scope.theme=appSettings.get('theme')||'default';$scope.editor_theme=appSettings.get('editor_theme')||'tomorrow';$scope.token=appSettings.get('token')||'';$scope.avatar=appSettings.get('avatar')||'';$scope.update_settings=function(){var data={};data.theme=$scope.theme;data.editor_theme=$scope.editor_theme;data.token=appSettings.get('token');data.avatar=appSettings.get('avatar');var saved=appSettings.set(data,function(response){if(response.status==='ok'){console.log('SAVED SETTINGS');window.location.reload();}else{console.log('NOT SAVED SETTINGS');}});};}
function viewCtrl($scope){$scope.avatar='https://secure.gravatar.com/avatar/'+JSON.parse(localStorage.settings).avatar;}
function loginCtrl($scope,ghAPI,appSettings){$scope.submit=function(){$scope.spinner=true;ghAPI.login($scope.user,$scope.pass,function(response){if(response.status===201){console.log(response);var data={};data.token=response.data.token;ghAPI.setToken(response.data.token);data.theme=appSettings.get('default')||'default';data.avatar=appSettings.get('none')||'none';data.editor_theme=appSettings.get('tomorrow')||'tomorrow';localStorage.settings=JSON.stringify(data);window.location.href='#/';}else{$scope.spinner=false;console.warn('[!!!] >>> Log-in failed - server responded with error.');$('.warn').slideDown('slow');$('.warn span').text('Log-in failed - server responded with error');setTimeout(function(){$('.warn').slideUp();},2500);}});};}
function logoutCtrl($scope,appSettings,gistData){$scope.logOut=function(){gistData.list=[];appSettings.logOut();};}
function listGistCtrl($scope,ghAPI,gistData){$scope.gists=gistData.list;if($scope.gists.hasOwnProperty('lastUpdated')){console.log($scope.gists.lastUpdated);var now=new Date();var seconds=Math.round((now.getTime()-$scope.gists.lastUpdated.getTime())/1000);console.log(seconds+' have passed since last LIST updated');if(seconds>60){ghAPI.gists();}}else{ghAPI.gists();}}
function singleGistCtrl($scope,$routeParams,gistData,ghAPI){$scope.gist=gistData.getGistById($routeParams.gistId);if($scope.gist.hasOwnProperty('single')&&$scope.gist.single.hasOwnProperty('lastUpdated')){console.log($scope.gist.single.lastUpdated);var now=new Date();var seconds=Math.round((now.getTime()-$scope.gist.single.lastUpdated.getTime())/1000);console.log(seconds+' have passed since last updated');if(seconds>60){ghAPI.gist($routeParams.gistId);}}else{ghAPI.gist($routeParams.gistId);}
$scope.copyToClipboard=function(data,message){message=message||'Content of a file <b>'+data.filename+'</b> copied to clipboard';if(clipboard!==undefined){clipboard.set(data.content||data,'text');}else{console.warn('>>> DEBUG MODE ON | Copy to clipboard really only works in App \n Data: '+(data.content||data));}
$('.ok').slideDown('slow');$('.ok span').html(message);setTimeout(function(){$('.ok').slideUp();},2500);};$scope.gotoSite=function(user,id,file){console.log('user',user);console.log('id',id);console.log('file',file);console.log('url','https://gist.github.com/'+user+'/'+id+'/#file-'+file.replace(/[.]/gi,'-'));gui.Shell.openExternal('https://gist.github.com/'+user+'/'+id+'/#file-'+file.replace(/[.]/gi,'-'));};$scope.enableEdit=function(old_object,old_description){$scope.old_object=old_object;$scope.old_description=old_description;console.log('enable edit');console.warn('Old files',old_object);console.warn('Old desc',old_description);$scope.edit=true;$('.edit').slideDown('slow');};$scope.disableEdit=function(){$scope.edit=false;};$scope.warnDeleteGist=function(){$('.delete').slideDown('slow');};$scope.cancelDeleteGist=function(){$('.delete').slideUp('slow');};$scope.star=function($event){if($event){$event.preventDefault();}
ghAPI.star($scope.gist.single.id,function(response){if(response.status===204){console.log(response);$('.ok').slideDown('slow');$('.ok span').text('Gist starred');$('.star').removeClass('icon-star-empty').addClass('icon-star');$scope.gist.single.starred=!$scope.gist.single.starred;$scope.gist.has_star=true;setTimeout(function(){$('.ok').slideUp();},2500);}else{console.log(response);$('.warn').slideDown('slow');$('.warn span').text('Gist not starred, something went wrong');setTimeout(function(){$('.warn').slideUp();},2500);}});};$scope.unstar=function($event){if($event){$event.preventDefault();}
ghAPI.unstar($scope.gist.single.id,function(response){if(response.status===204){console.log(response);$('.ok').slideDown('slow');$('.ok span').text('Star removed');$('.star').removeClass('icon-star').addClass('icon-star-empty');$scope.gist.single.starred=!$scope.gist.single.starred;$scope.gist.has_star=false;setTimeout(function(){$('.ok').slideUp();},2500);}else{console.log(response);$('.warn').slideDown('slow');$('.warn span').text('Something went wrong');setTimeout(function(){$('.warn').slideUp();},2500);}});};$scope.del=function($event){if($event){$event.preventDefault();}
console.log($scope);ghAPI.delete($scope.gist.single.id,function(response){if(response.status===204){$('.ok').slideDown('slow');$('.ok span').text('Gist deleted');gistData.list.splice(gistData.list.indexOf(gistData.getGistById($scope.gist.single.id)),1);setTimeout(function(){$('.ok').slideUp();window.location.href='index.html#/';},1000);}else{console.log(response);$('.warn').slideDown('slow');$('.warn span').text('Gist not deleted, something went wrong');setTimeout(function(){$('.warn').slideUp();},2500);}});};$scope.addFile=function(){var fileName='newFile'+Object.keys($scope.gist.single.files).length+'.txt';$scope.gist.single.files[fileName]={content:'',filename:fileName,language:'text'};$scope.enableEdit();};$scope.dragStart=function(e){e.stopPropagation();e.preventDefault();$('.edit').slideDown('slow');$('.main section').addClass('dragarea');$('.edit span').text('Drag detected - now drop!');console.log('dragging start');};$scope.drop=function(e){e.stopPropagation();e.preventDefault();var data=event.dataTransfer;for(var i=0;i<data.files.length;i++){var file=data.files[i];var reader=new FileReader();$('.edit').slideUp('slow');$('.ok').slideDown('slow');$('.main section').removeClass('dragarea');$('.ok span').html('Dropped: <b>'+file.name+'</b>');$scope.edit=true;reader.onloadend=(function(filename){return function(event){$scope.gist.single.files[filename]={filename:filename,content:event.target.result,language:'html'};$scope.$digest();};})(file.name);reader.readAsText(file);}};$scope.dragEnd=function(e){e.stopPropagation();e.preventDefault();console.log('drag end');};$scope.deleteFile=function(file_name){console.log('delete file',file_name);$('.loading span').html('Deleting file <b>'+file_name+'</b>');var data={description:$scope.gist.description,id:$scope.gist.id,files:{}};for(var file in $scope.gist.single.files){data.files[file]={content:$scope.gist.single.files[file].content,filename:$scope.gist.single.files[file].filename};}
data.files[file_name]=null;delete $scope.gist.single.files[file_name];ghAPI.edit($scope.gist.single.id,data,function(response){if(response.status===200){$('.ok').slideDown('slow');$('.ok span').html('File '+file_name+' removed');$scope.edit=false;$scope.gist.single.history=response.data.history;$scope.gist.filesCount=Object.keys($scope.gist.single.files).length;console.warn(response.data.id);setTimeout(function(){$('.ok').slideUp();},2500);}else{$('.warn').slideDown('slow');$('.warn span').text('Something went wrong');setTimeout(function(){$('.warn').slideUp();},2500);}});};$scope.update=function(){$('.loading span').text('Saving...');$('.edit').slideUp();var data={description:$scope.gist.description,id:$scope.gist.id,files:{}};for(var file in $scope.gist.single.files){data.files[file]={content:$scope.gist.single.files[file].content,filename:$scope.gist.single.files[file].filename};}
console.warn('New files',$scope.gist.single.files);console.warn('New desc',$scope.gist.description);if(angular.equals($scope.old_object,$scope.gist.files)||angular.equals($scope.old_description,$scope.gist.description)){$('.warn').slideDown('slow');$('.warn span').text('No changes to save.');$scope.edit=false;setTimeout(function(){$('.warn').slideUp();},2500);}else{ghAPI.edit($scope.gist.single.id,data,function(response){if(response.status===200){$('.ok').slideDown('slow');$('.ok span').text('Gist saved');$scope.edit=false;$scope.gist.single.files=response.data.files;$scope.gist.single.history=response.data.history;$scope.gist.tags=$scope.gist.description?$scope.gist.description.match(/(#[A-Za-z0-9\-\_]+)/g):[];$scope.gist.filesCount=Object.keys($scope.gist.single.files).length;setTimeout(function(){$('.ok').slideUp();},2500);}else if(response.status===422){$('.warn').slideDown('slow');$('.warn span').text('You cannot save empty files');setTimeout(function(){$('.warn').slideUp();},2500);}else{$('.warn').slideDown('slow');$('.warn span').text('Something went wrong');setTimeout(function(){$('.warn').slideUp();},2500);}});}};}
function commentsGistCtrl($scope,$routeParams,ghAPI){if($scope.gist.comments>0){ghAPI.comments($routeParams.gistId,function(response){if(response.status===200){$scope.comments=response.data;}else{console.warn('[!!!] >>> Comments not loaded - server responded with error.');}});}}
function createGistCtrl($scope,ghAPI,gistData){$scope.description='';$scope.isPublic=false;$scope.files=[{filename:'',content:''}];$scope.addFile=function(){console.log('add file');console.log($scope);$scope.files.push({content:'',filename:'',language:'html'});};$scope.save=function($event){if($event){$event.preventDefault();}
