...
return items;};});'use strict';angular.module('gisto',['ui','gisto.filters','gisto.services','gisto.directives','JobIndicator','gitHubAPI','gistData','appSettings']).config(['$routeProvider',function($routeProvider){$routeProvider.when('/',{templateUrl:'partials/empty.html'});$routeProvider.when('/login',{templateUrl:'partials/login.html',controller:loginCtrl});$routeProvider.when('/settings',{templateUrl:'partials/settings.html'});$routeProvider.when('/gist/:gistId',{templateUrl:'partials/single-gist.html',controller:singleGistCtrl});$routeProvider.when('/create',{templateUrl:'partials/create.html',controller:createGistCtrl});$routeProvider.otherwise({redirectTo:'/'});}]);'use strict';angular.module('gisto.services',[]);angular.module('JobIndicator',[]).config(function($httpProvider){var numLoadings=0;$httpProvider.responseInterceptors.push(function(){return function(promise){numLoadings++;$('.loading').show();var hide=function(r){if(!(--numLoadings)){$('.loading').slideUp();}
